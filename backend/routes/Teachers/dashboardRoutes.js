...
// const { auth, authorize } = require('../../middleware/auth'); // Using existing auth middleware
...
// /**
//  * @route PUT /api/student/update-activity/:id
//  * @desc Update status of an activity
//  * @access Private (teacher only)
//  */
// router.put('/update-activity/:id', auth, authorize('teacher', 'guro'), async (req, res) => {
//   try {
//     const activityId = req.params.id;
//     const { status } = req.body;

//     // In a real implementation, we would update a specific activity in the database
//     // For now, we'll just return a success message
    
//     console.log(`Activity ${activityId} marked as ${status}`);
    
//     return res.json({
//       success: true,
//       message: `Activity ${activityId} has been updated to ${status}`,
//       activity: {
//         id: activityId,
//         status: status
//       }
...
// routes/Teachers/dashboardRoutes.js - Enhanced version with additional API endpoints
const express = require('express');
const mongoose = require('mongoose');
 ...
 * @access Private (teacher only)
 */
router.get('/data', auth, authorize('teacher', 'guro'), async (req, res) => {
  try {
    // Access test database for student data
    const testDb = mongoose.connection.useDb('test');
    const usersCollection = testDb.collection('users');
    ...
    // Fetch students
    const studentsData = await usersCollection.find({}).toArray();
    
    // Process students for dashboard display
    const processedData = processStudentData(studentsData);
    ...
    const readingLevelDistribution = readingLevelAggregation
      .filter(level => level._id) // Filter out empty levels
      .map(level => ({
        name: level._id,
        value: level.count,
        color: getReadingLevelColor(level._id)
      }));
    ...
    // Generate pendingActivities
    const pendingActivities = generateActivities(processedData.allStudents);
    
    // Generate progress data
    const progressData = generateProgressData(readingLevelDistribution);
    
    // Generate prescriptive data
    const prescriptiveData = generatePrescriptiveData(readingLevelDistribution);
...
/**
 * @route GET /api/dashboard/reading-level-distribution
 * @desc Get distribution of students by reading level
 * @access Private (teacher only)
 */
router.get('/reading-level-distribution', auth, authorize('teacher', 'guro'), async (req, res) => {
  try {
    ...
    const distribution = readingLevelAggregation
      .filter(level => level._id) // Filter out empty levels
      .map(level => ({
        name: level._id,
        value: level.count,
...
/**
 * @route GET /api/dashboard/students-needing-attention
 * @desc Get list of students needing attention
 * @access Private (teacher only)
 */
router.get('/students-needing-attention', auth, authorize('teacher', 'guro'), async (req, res) => {
  try {
    ...
    // Find students needing attention
    const studentsData = await usersCollection.find({
      $or: [
        { readingLevel: "Not Assessed" },
        { readingLevel: "Early" },
        { readingPercentage: { $lt: 70 } }
    ...
    // Process students
    const students = processStudentData(studentsData).allStudents;
 ...
 * @access Private (teacher only)
 */
router.get('/activities', auth, authorize('teacher', 'guro'), async (req, res) => {
  try {
    const testDb = mongoose.connection.useDb('test');
    const usersCollection = testDb.collection('users');
    
    // Find students needing attention to generate activities
    const studentsData = await usersCollection.find({
      $or: [
        { readingLevel: "Not Assessed" },
        { readingLevel: "Early" },
    ...
    // Generate activities
    const activities = generateActivities(processedData.allStudents);
...
/**
 * @route GET /api/dashboard/prescriptive-data
 * @desc Get prescriptive data for reading levels
 * @access Private (teacher only)
 */
router.get('/prescriptive-data', auth, authorize('teacher', 'guro'), async (req, res) => {
  try {
    ...
    // Generate prescriptive data
    const prescriptiveData = generatePrescriptiveData(distribution);
    
    return res.json(prescriptiveData);
  } catch (error) {
    console.error('Error fetching prescriptive data:', error);
    return res.status(500).json({ message: 'Server error', error: error.message });
  }
});

/**
 * @route GET /api/dashboard/by-section/:section
 * @desc Get students by section
 * @access Private (teacher only)
 */
router.get('/by-section/:section', auth, authorize('teacher', 'guro'), async (req, res) => {
  try {
...
/**
 * @route PUT /api/dashboard/update-activity/:id
 * @desc Update activity status (e.g., mark as resolved)
 * @access Private (teacher only)
 */
router.put('/update-activity/:id', auth, authorize('teacher', 'guro'), async (req, res) => {
  try {
...
// Helper function to get color for reading level
...
// Helper function to determine improvement categories
function determineCategoriesForImprovement(readingLevel) {
  switch(readingLevel) {
    case 'Early':
...
function generateActivities(students) {
...
// Helper function to generate progress data (mockup data for visualization)
function generateProgressData(levelDistribution) {
  // Create progress data for each reading level
...
// Helper function to generate prescriptive data
function generatePrescriptiveData(levelDistribution) {
